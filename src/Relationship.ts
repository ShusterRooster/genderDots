...
export class Relationship {
  ...
  static readonly relationshipTypes: string[] = [
    "seek",
    "orbit",
    "chain",
    "merge",
  ...
  static readonly stealChance: number = 5;
  ...
  maxPartners = Math.floor(random(2, Relationship.maxPartners + 1));
  ...
  seekPartners() {
    ...
    for (const shape of arr) {
      if (determineProb(Relationship.stealChance)) {
  ...
  run() {
    ...
    else if (this.relationshipType == "chain") this.drawChains();
    else if (this.relationshipType == "chain") this.merge();
  }
  ...
  getAvgColor() {
    ...
    for (let i = 0; i < this.partners.length - 1; i++) {
      ...
      if (typeof totalColor == undefined) totalColor = curAvg;
      else totalColor = totalColor!.add(curAvg).divide(i);
    }
