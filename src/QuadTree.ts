...
export class Quadtree {
    ...
    getIndex(obj: paper.PathItem): number[] {

        const indexes: number[] = [],
            boundsCenterX = this.bounds.x + (this.bounds.width / 2),
            boundsCenterY = this.bounds.y + (this.bounds.height / 2);

        const startIsNorth = obj.position.y < boundsCenterY,
            startIsWest = obj.position.x < boundsCenterX,
            endIsEast = obj.position.x + obj.bounds.width > boundsCenterX,
            endIsSouth = obj.position.y + obj.bounds.height > boundsCenterY;
     ...
     */
    insert(obj: paper.PathItem): void {
     ...
     */
    retrieve(obj: paper.PathItem): paper.PathItem[] {
     ...
     */
    remove(obj: paper.PathItem, fast = false): boolean {
     ...
     * @param fast - Set to true to increase performance temporarily by preventing cleanup of empty subnodes (optional, default: false).
     */
    update(obj: paper.PathItem, fast = false): void {
